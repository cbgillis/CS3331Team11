...
public class EventSeatingMap{
	...
	/********************************/
	private Seats[] movieSeats = new Seats[60];
	private Seats[][] airSeats;
	private Seats[] theaterSeats = new Seats[70];
	...
	private void createMovieTheater(){
		...
		for(int i = 0; i < 60; i++){
			...
			if(i >= 20 && i < 45){			//The more expensive seats 15-40.
				seats[seatCounter] = new Seats(8.00, "B" + i);
				seats[seatCounter].setText(seats[seatCounter].getSeatName());
				seats[seatCounter].setActionCommand(Integer.toString(seatCounter));
				seats[seatCounter].addActionListener(new ButtonClickListener());
				movieSeats[seatCounter] = seats[seatCounter];
			...
			if(i >= 45 && i < 50){			//Space between balcony and reg seats
				panel.add(new JLabel(""));
			}
			if(i >= 50 && i < 60){			//The most expensive seats 40-49.
				seats[seatCounter] = new Seats(10.00, "C" + i);
				seats[seatCounter].setText(seats[seatCounter].getSeatName());
				seats[seatCounter].setActionCommand(Integer.toString(seatCounter));
				seats[seatCounter].addActionListener(new ButtonClickListener());
				movieSeats[seatCounter] = seats[seatCounter];
		...
		screen.add(sc);
		frame.add(panel);
		frame.add(screen, BorderLayout.NORTH);				//Add the screen to the bottom of the frame.
	...
	/********************************/
	/* Creates stage theater layout*/
	private void createStageTheatre(){
		isTheatre = true;
		int seatCounter = 0;							
		Seats[] seats = new Seats[75];						
		JPanel panel = new JPanel();															
		JFrame frame = new JFrame("Stage Theater");			
		frame.setSize(1200, 1200);
		panel.setLayout(new GridLayout(13, 7));

		//rows of seats and spaces between
		for(int i = 0;i < 5;i++ ){
			panel.add(new JLabel("Column " + i));
		}
		for(int i = 0; i < 65; i++){
			if(i < 15){	//regular seats
				seats[seatCounter] = new Seats(50.00, "A" + i);
				theaterSeats[seatCounter] = seats[seatCounter];
				seats[seatCounter].setText(seats[seatCounter].getSeatName());
				seats[seatCounter].setActionCommand(Integer.toString(seatCounter));
				seats[seatCounter].addActionListener(new ButtonClickListener());
				panel.add(seats[seatCounter++]);
			...
			if(i >= 20 && i < 35){//regular seats
				seats[seatCounter] = new Seats(150.00, "B" + i);
				seats[seatCounter].setText(seats[seatCounter].getSeatName());
				seats[seatCounter].setActionCommand(Integer.toString(seatCounter));
				seats[seatCounter].addActionListener(new ButtonClickListener());
				movieSeats[seatCounter] = seats[seatCounter];
			...
			if(i >= 35 && i < 45){// regular space between balcony seats
				panel.add(new JLabel(""));
			}
			if(i >= 45 && i < 60){//Balcony Seating
				seats[seatCounter] = new Seats(200.00, "C" + i);
				seats[seatCounter].setText(seats[seatCounter].getSeatName());
				seats[seatCounter].setActionCommand(Integer.toString(seatCounter));
				seats[seatCounter].addActionListener(new ButtonClickListener());
				movieSeats[seatCounter] = seats[seatCounter];
	...
	private void createAirline(){
		...
		Seats[][] seats = new Seats[6][12];//creates a 2d array of uninitialized seat buttons
		...
		for(int i = 0; i < 6; i++){
			...
			{
				if(j < 4 && i != 3){
					seats[i][j] = new Seats(200.00, "A" + i+j + "");//first class
				...
				if((j >= 4 && j < 8) && i != 3){
					seats[i][j] = new Seats(150.00, "B" + i+j + "");//business
					seats[i][j].setActionCommand(Integer.toString(i+j));
					seats[i][j].addActionListener(new ButtonClickListener());
					seats[i][j].setText(seats[i][j].getSeatName());
					//seats[i][j].addActionListener(this);
				...
				if(j >= 8 && i != 3){
					seats[i][j] = new Seats(100.00, "C" + i+j + "");//economy
					//seats[i][j].addActionListener(this);
					seats[i][j].setActionCommand(Integer.toString(i+j));
					seats[i][j].addActionListener(new ButtonClickListener());
					seats[i][j].setText(seats[i][j].getSeatName());
